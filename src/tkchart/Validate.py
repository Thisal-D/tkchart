...
class Validate:
    ...
    def _isFloat(value: any, var: str) -> None:
        if type(value) != float:
            raise TypeError(f"{Validate._var_font(var)} {Validate._error_font("must be float.")}")
    ...
    def _isNumeric(value: int, var: str) -> None:
        if type(value) == int or type(value) == float:
            ...
            raise TypeError(f"{Validate._var_font(var)} {Validate._error_font("must be int or float.")}")
    ...
    def _isValidColor(value: any, var: str) -> None:
        ...
        try:
            ...
            raise ColorError(f"{Validate._var_font(var)} {Validate._error_font("must be valid color. eg:- '#ff0000'/ 'red'")}" )
    ...
    def _isValidFont(value: any, var: str) -> None:
        ...
        try:
            ...
            raise FontError(f"{Validate._var_font(var)} {Validate._error_font("must be valid font. eg:- ('arial',10,'bold')")}")
        

    def _isValidFunction(value: any, var: str) -> None:
        if not callable(value) and  value != None:
            raise FunctionError(f"{Validate._var_font(var)} {Validate._error_font("must be function with two parameters or *args.")}")
    ...
    def _isValidStyleType(value: any, var: str) -> None:
        ...
        if len(value) == 2:
            ...
            raise LengthError(f"{Validate._var_font(var)} {Validate._error_font("length must be two.")}")
    ...
    def _isValidYAxisValues(value: any, var: str) -> None:
        Validate._isTuple(value, var)
        if len(value) == 2:
            if type(value[0]) == int or type(value[0]) == float and type(value[1]) == int or type(value[1]) == float:
                if value[0] < value[1]:
                    ...
                else:
    ...
    def _isValidYAxisMaxValue(value: any, var: str) -> None:
        ...
        if value == 0:
            raise TypeError(f"{Validate._var_font(var)} {Validate._error_font("must be less than 0 or bigger than 0")}")